...
INSTRUCTIONS_WIDTH = 64
INSTRUCTIONS_LABEL = (
    "Pour chaque personnage du jeu :\n"
    "- saisissez le nom (12 caractères maximum, format ASCII) ;\n"
    "- choisissez l'image (format BMP, JPEG ou PNG).\n"
    "Note :\n"
    "Laissez un champ vide pour conserver l'original (nom ou image)."
)
...
class Character(WidgetFrame):
    def __init__(self, kind, row, column, window):
        ...
        self.set_position(row, column, 1, 2)
...
class Window(Tk):
    def __init__(self):
        ...
        self.enemy_to_catch = Character(kind_enemy_to_catch, 2, 0, self)
        kind_enemy_to_avoid = Const.character.enemy_to_avoid
        self.enemy_to_avoid = Character(kind_enemy_to_avoid, 3, 0, self)
        self.obstacle = Character(Const.character.obstacle, 4, 0, self)

        self.button_ok = WidgetButton(self)
        self.button_ok.set_position(5, 0)
        self.button_ok.set_sticky(E)
        self.button_ok.set_command(BUTTON_OK, self.command_ok)
        self.button_cancel = WidgetButton(self)
        self.button_cancel.set_position(5, 1)
